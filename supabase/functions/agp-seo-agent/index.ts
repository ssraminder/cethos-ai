...
Agency: Ascelo AI — serves clients globally. 5 languages: EN, AR, FR, HI, PA.
...
serve(async (req) => {
  try {
    ...
    for (const page of KEY_PAGES) {
      ...
      let pageHtml = ''
      try {
        const res = await fetch(pageUrl, { headers: { 'User-Agent': 'AsceloAISEOBot/1.0' } })
    ...
    const emailHtml = `
<h1>📊 Monthly SEO Audit Report — Ascelo AI</h1>
...
<p style="color:#999;font-size:12px;">Generated by Ascelo AI SEO Agent • ${new Date().toISOString()}</p>
    `
    ...
    await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender: { name: 'Ascelo AI SEO Agent', email: 'noreply@ascelo.ai' },
        to: [{ email: ceoEmail }],
